...
class CustomDataset(Dataset):
    def __init__(self, image_path, metadata_path, mode, transform, num_val=100):
        ...
        if mode == 'test' or 'train':
            if 'cambridge' in self.image_path:
                self.lines = raw_lines[3:]
            elif '7scenes' in self.image_path:
                self.lines = raw_lines[1:]
            else:
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            file_name = self.train_filenames[index]
        elif self.mode in ['val', 'test']:
            image = Image.open(self.test_filenames[index])
            pose = self.test_poses[index]
            file_name = self.test_filenames[index]

        return self.transform(image), torch.Tensor(pose), file_name
