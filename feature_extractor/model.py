...
def model_parser(model, fixed_weight=False, dropout_rate=0.0, bayesian = False):
    ...
    if model == 'Googlenet':
        ...
        network = Efficientnet_b0(base_model, fixed_weight)
    else:
...
class Efficientnet_b0(nn.Module):
    def __init__(self, base_model, fixed_weight=False, dropout_rate = 0.1):
        ...
        self.fc_rotation = nn.Linear(1024, 4, bias=False)
        init_modules = [self.fc_last, self.fc_position, self.fc_rotation]
