...
def main(parser):
    ...
    for name,group in groups:
        ...
        for threshold in simi_threshold_list:
            ...
            norm_ori_error.append(len(error_list))
        norm_max = max(norm_ori_error)
        norm_ori_error  = np.array(norm_ori_error)/norm_max*100
        ax1.plot(simi_threshold_list,norm_ori_error,label = name)
    ax1.legend()
    ax1.set_xlabel("Similarity threshold", fontsize=35)
    ax1.set_ylabel("Ratio of retained predictions (%)",fontsize=35)
    #plt.tight_layout()
    ax1.set_ylim(0, 100)
    ...
    plt.savefig(f'../{apr}/results/ratio_correlation_line.pdf')
